...
# Исправлено: теперь ссылки могут содержать параметры (?si=...) и заканчиваются не только на символы без спецзнаків
...
def admin_only(func):
    async def wrapper(message: types.Message, *args, **kwargs):
        ...
        if message.from_user.id not in ADMIN_IDS:
            await message.reply("Ви не маєте прав для цієї команди.")
            return
...
def owner_only(func):
    async def wrapper(message: types.Message, *args, **kwargs):
        ...
        if message.from_user.id != OWNER_ID:
            await message.reply("Тільки власник може виконати цю команду.")
            return
        return await func(message, *args, **kwargs)
    return wrapper
...
async def send_welcome(message: types.Message):
    await message.reply("Привет! Пришли мне ссылку на TikTok, Instagram Reels или YouTube Shorts, и я отправлю тебе видео.")

async def send_help(message: types.Message):
    await message.reply(
        "Этот бот скачивает видео из TikTok, Instagram Reels и YouTube Shorts.\n"
...
async def admin_stats(message: types.Message):
    ...
    await message.reply(
        f"Статистика:\n"
        f"Оброблено відео: {stats.get('processed_videos', 0)}\n"
        f"Розмір кешу: {cache_size // (1024*1024)} МБ\n"
        f"Унікальних користувачів: {len(get_users())}\n"
        f"Груп: {len(get_groups())}"
    )

@admin_only
async def admin_clear_cache(message: types.Message):
    clear_cache_on_start()
    await message.reply("Кеш очищено.")
    ...
    if users:
        ...
        for uid in users:
            try:
                ...
                user_lines.append(f"{uid} - User")
        await message.reply("Унікальні користувачі:\n" + "\n".join(user_lines))
    else:
        await message.reply("Ще немає користувачів.")
    ...
    if groups:
        ...
        for gid in groups:
            try:
                ...
                group_lines.append(f"{gid} - Group")
        await message.reply("Групи:\n" + "\n".join(group_lines))
    else:
        await message.reply("Ще немає груп.")
...
async def admin_help(message: types.Message):
    await message.reply(
        ...
        "/ban_user USER_ID — заблокувати\n"
        "/unban_user USER_ID — розблокувати\n"
        "/logs N — останні N рядків логів\n"
...
async def handle_video_link(message: types.Message):
    ...
    if cache_size > stats.get("max_cache_size", max_cache_size):
        logger.info("Кеш перевищує ліміт, очищаємо...")
        clear_cache_on_start()
    ...
    async def schedule_file_removal(path):
        ...
        try:
            if os.path.exists(path):
                os.remove(path)
                logger.info(f"Файл {path} видалений автоматично через 10 хвилин.")
        except Exception as e:
            logger.warning(f"Помилка при автоудаленні файлу: {e}")
    ...
    progress = {'percent': 0}
    progress_msg = await message.reply("Скачую відео, зачекайте...")
        ...
        if d['status'] == 'downloading':
            ...
            if total:
                ...
                if abs(percent - progress.get('percent', 0)) >= 1:
                    ...
                    # Викликаємо update_progress_message через loop
    ...
    try:
        ...
        await bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=progress_msg.message_id,
            text="Відео завантажено, відправляю..."
        )
        ...
        try:
            ...
            logger.warning(f"Не вдалося видалити повідомлення: {ex}")
